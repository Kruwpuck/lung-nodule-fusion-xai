...
> **Status per 8 Agustus 2026.** Dokumen ini merekam keadaan di akhir Rev1 dan sengaja
> dibiarkan begitu, karena nilainya justru sebagai catatan sejarah. Dua hal sudah
> digantikan sesudahnya. Pertama, angka ablasi fusi di Bab 6.3 diukur sebelum bug resolusi
> `input_size` diperbaiki dan sebelum nested CV, jadi angka final ada di §6.1
> `LAPORAN_TRACK1_FUSION_XAI.md`. Kedua, kesimpulan Track 1 sudah dipersempit: radiomics
> mengalahkan fusi **berparameter**, sedangkan `fusion_late` setara dengannya dan menang
> telak atas `cnn_only` (§6.3 laporan yang sama). Bab 6.2 tentang stabilitas hyperparameter
> Track 2 tidak terpengaruh dan tetap berlaku penuh.
